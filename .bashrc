...
# ADDED
# adds git prompt: from https://github.com/jcgoble3/gitstuff/tree/master
git_branch() {
...
# bmsgrep [PATTERN]: use apparix bmgrep. do git status on each bm grepped by PATTERN
# do not use "" around bmgrep $var to ignore arguments that are more than one (bmgrep default behavior)
bms() {
  bmgrep $1 | awk '{print $2}' | xargs -i sh -c 'cd {} && echo && \
    echo "\033[0;96m-----------------------" && \
    echo {} && \
    echo "\033[0m------------------------" && \
